    ...
    methods: {
        ...
        validateId() {
            ...
            const existingBrand = this.brands.find(
                b => b.id === this.form.id && (!this.isEditing || b.id !== this.originalId)
            );
            if (existingBrand) {
        ...
        async updateBrand() {
            try {
                const response = await axios.put(
                    `http://127.0.0.1:8000/api/brands/${this.originalId}`,
                    this.form
                );
                const index = this.brands.findIndex(b => b.id === this.originalId);
                if (index !== -1) this.brands[index] = response.data;
